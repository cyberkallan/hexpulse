...
void drawWebUiScreen(bool mode_ap) {
    ...
    if (mode_ap) {
        ...
        tft.drawCentreString("HexPulse/hexpulse", tftWidth / 2, 7, 1);
    }
    setTftDisplay(0, 0, ALCOLOR, FM);
    tft.drawCentreString("HexPulse WebUI", tftWidth / 2, 27, 1);
    String txt;
    if (!mode_ap) txt = WiFi.localIP().toString();
    else txt = WiFi.softAPIP().toString();
    tft.setTextColor(bruceConfig.priColor);

    tft.drawCentreString("http://hexpulse.local", tftWidth / 2, 45, 1);
    setTftDisplay(7, 67);
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists("/HexPulseWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/HexPulseWebUI/" + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, "/HexPulseWebUI/" + filename, contentType);
    } else {
