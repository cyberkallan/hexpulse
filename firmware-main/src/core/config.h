...
public:
    ...
    const char *filepath = "/hexpulse.conf";
    ...
    // Wifi
    Credential webUI = {"admin", "hexpulse"};
    std::vector<String> webUISessions = {}; // FIFO queue of session tokens
    WiFiCredential wifiAp = {"HexPulse", "hexpulse"};
    ...
    std::vector<QrCodeEntry> qrCodes = {
        {"HexPulse AP",   "WIFI:T:WPA;S:HexPulse;P:hexpulse;;"},
        {"HexMind",       "https://hexmind.space"},
        {"HexPulse Repo", "https://github.com/cyberkallan/hexpulse"},
        {"Creator",       "https://instagram.com/imarjunarz"}
    };
